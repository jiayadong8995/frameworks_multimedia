...
typedef struct rtc_conver_gui_s
{
    uv_loop_t ui_loop;
    void* handle;
...
{
    int ret = 0;
    rtc_conver_gui_t* ai_gui = (rtc_conver_gui_t*)lv_event_get_user_data(e);
    if(ai_gui == NULL) {
        return;
    }
...
static void ui_create(rtc_conver_gui_t* arg)
{
    if (arg == NULL)
    {
        return;
    }
    rtc_conver_gui_t* ai_gui = arg;
    //TODO: 字体需要优化 
    // 1. 字体资源（上实际开发板后优化）
    // 2. 字体大小（上实际开发板后优化）
    LV_IMG_DECLARE(mic);
...
{
    rtc_conver_gui_t* ai_gui = (rtc_conver_gui_t*)cookie;
...
static int ai_conversation_engine_init(rtc_conver_gui_t* arg)
{
